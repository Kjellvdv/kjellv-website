...
Drafts go in `drafts/`, which is gitignored. The repo is public, so a committed draft is a published draft. A case only moves up out of `drafts/` once it's approved, and the schema refuses `status: published` without a `published_at` date. The "last updated" month on each case page comes from `updated` in `src/data/cases.ts`.
